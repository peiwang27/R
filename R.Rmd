...
```
- geom_bar()函数绘制
ggplot函数绘制图形所使用的数据为data.frame，因为VADeaths数据集的格式为matrix，因此首先使用melt()函数将matrix转化为data.frame.
调节条形图的样式使用geom_bar()函数的position参数.
```{r geom_bar()函数}
data(mpg)
ggplot(mpg, aes(class)) + geom_bar()

VA.melt <- melt(VADeaths)
ggplot(VA.melt, aes(x=Var1, y=value, fill=Var2)) +
